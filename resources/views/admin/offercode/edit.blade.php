...
<?php
  $costReq = 'required';
  if($get_det->offerType == 'PERCENT'){
    if($get_det->percentType == 'FLAT'){
      $costReq ='';
    }
  }
  
  $percentReq = 'required';
  if($get_det->offerType == 'FIXED'){
    $percentReq = '';
  }
?>
          ...
          <form data-toggle="validator" role="form" action="{{ route('saveOffer') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group">
                ...
                <select name='offerType' id="offer_type" class="form-control" data-required-error="Please select offer type" @if(isset($get_det->offerType)) disabled="" @endif>
                  <option value="">Select Offer Type</option>
                   <option value="FIXED" @if(isset($get_det->offerType) && $get_det->offerType == 'FIXED') selected @endif>Fixed</option>
                   <option value="PERCENT" @if(isset($get_det->offerType) && $get_det->offerType == 'PERCENT') selected @endif>Percent</option>
                </select>
                 ...
                 @endif
                 <div class="help-block with-errors"></div>
                 <input type="hidden" name="offer_type" value="{{ $get_det->offerType }}">
              </div>

              <div class="form-group" id="percentTypeDiv" {{(!isset($get_det->percent) ? "display:none;" : '')}}>
                <label>Percent Type</label><span class="star">&nbsp;*</span >
                <select name='percentType' id="percent_type" class="form-control" data-required-error="Please select percent type" @if(isset($get_det->percentType)) disabled="" @endif>
                  <option value="FLAT" @if(isset($get_det->percentType) && $get_det->percentType == 'FLAT') selected @endif>Flat</option>
                  <option value="UPTO" @if(isset($get_det->percentType) && $get_det->percentType == 'UPTO') selected @endif>Up to</option>
                </select>
                 ...
                 @endif
                 <div class="help-block with-errors"></div>
                 <input type="hidden" name="percent_type" value="{{ $get_det->percentType }}">
              </div>

              <div class="form-group" id="percentDiv" style={{(!isset($get_det->percent) ? "display:none;" : '')}}>
                <label>Percent</label><span class="star">&nbsp;*</span ><small> In (%)</small>
                <input  onkeypress="return isNumberKey(event);"  name='offer_percent' id="offer_percent" class="form-control" value="{{  (!empty($get_det->id) ? $get_det->percent :old('percent')) }}" placeholder="Enter Offer Percent" data-required-error="Please select offer percent" <?php echo ($percentReq != '') ? $percentReq : '';?>>
                 @if ($errors->has('plan_cost'))
                    <span class="star help-block">
                        <strong>{{ $errors->first('plan_cost') }}</strong>
                    </span>
              ...
              <div class="form-group" id="costDiv" style={{ (isset($get_det->amount) ? '' : 'display:none;') }}>
                <label>Offer Cost</label><span class="star">&nbsp;*</span ><small> In (R)</small>
                <input  onkeypress="return isNumberKey(event);"  name='offer_cost' id="offer_cost" class="form-control" value="{{  (!empty($get_det->id) ? $get_det->amount :old('plan_cost')) }}" placeholder="Enter Offer Cost" data-required-error="Please select offer cost" <?php echo ($costReq != '') ? $costReq : '';?>>
                 @if ($errors->has('plan_cost'))
                    <span class="star help-block">
                        <strong>{{ $errors->first('plan_cost') }}</strong>
                    </span>
