      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <!-- /.box-header -->
              <div class="box-body">
            <form class="form-inline" action="<?php echo url('/') ?>/saveInvitecode" data-toggle="validator" method="post" autocomplete="off">
              {{ csrf_field() }}
              <div class="input-group input-group-xs">
                     <input name="invitation_setting" data-required-error="Please enter your Offer Code"  required id="code" value="<?php echo (!empty($referal_offer_days->value) ? $referal_offer_days->value : '' ) ?>" type="text" class="form-control"  id="textInput" placeholder="Number Of days">
                  </div>
                <div class="form-group">
                     <button type="submit" class="btn btn-flat margin" data-toggle="tooltip" title="Search">Save</button>
                    <?php if(!empty($referal_offer_days->is_active) && $referal_offer_days->is_active=='1'){  ?> <button class="btn btn-success btn-xs btn-flat chng-sta" data-status="0" data-id="{{ $referal_offer_days->id }}"> Active</button>  <?php }else{?><button class="btn btn-danger btn-xs btn-flat chng-sta" data-status="1" data-id="{{ (!empty($referal_offer_days->id) ? $referal_offer_days->id :"") }}">Deactive</button><?php } ?>
                </div>
            </form>
...
@section('js-script')
   ...
   });   $('.chng-sta').click(function(event){
      let click = this;
      let x = confirm("Do you realy want to change status?");
        if(x){
            $.ajax({
             type: 'put',
             url : '{{ route('changeInvitecodeSetting') }}',
             data : {
                'id' :  $(this).attr('data-id'),
                'status' :  $(this).attr('data-status'),
                '_token': '{{ csrf_token() }}'
